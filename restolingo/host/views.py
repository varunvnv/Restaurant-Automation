...
def index(request):  # TO DO: add authorization
    if access_restricted(request, 'host'):
        ...
        if request.method == 'POST':
            ...
            table_to_update.status = 'occupied'
            table_to_update.save()
            table_list = Table.objects.all()
            template = loader.get_template('host/tables.html')
            context = Context({'table_list': table_list,'shift':shift})
